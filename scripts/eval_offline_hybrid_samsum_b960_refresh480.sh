...
LIMIT=${LIMIT:-100}
OUTPUT=${OUTPUT:-results/offline_hybrid_samsum_b960_refresh480_20260811/limit${LIMIT}}
...
.venv/bin/python evaluation_script.py \
  ...
  --batch_size 1 \
  --limit "${LIMIT}" \
  --model_args "pretrained=${MODEL},is_feature_cache=False,is_cfg_cache=False,max_length=2048,student_path=${KEEP_STUDENT},student_refresh_path=${DELTA_STUDENT},student_prompt_layer_split=True,student_budget=960,student_frozen_layers=0,student_refresh_tokens=480,student_question_window=128,student_score_activation=softmax" \
