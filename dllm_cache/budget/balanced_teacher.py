...
from dllm_cache.budget.online_teacher import OnlineTeacherConfig, generate_with_online_teacher
...
DEFAULT_OUTPUT_ROOT: Final = Path("results/budget/online_self_generated_teacher_balanced_2k_g32")
...
def main() -> int:
    ...
    samples = load_balanced_samples(config.data_path, config.n_samples)
    saved = 0
    t0 = time.time()
    for index, sample in enumerate(samples, start=1):
        ...
        saved += 1
        print(
            f"[online-teacher-balanced {index}/{len(samples)}] saved={out_path} "
            f"dataset={sample.dataset} prompt={example.prompt_length} "
            f"commits={rec['commit_count']} generated={rec['generated_answer']!r}",
            flush=True,
        )
...
def load_balanced_samples(path: Path, limit: int) -> list[BalancedSample]:
    samples: list[BalancedSample] = []
    with path.open("r", encoding="utf-8") as handle:
        for index, line in enumerate(handle):
            if limit > 0 and len(samples) >= limit:
                break
            samples.append(parse_balanced_sample(json.loads(line), index))
    return samples


def parse_balanced_sample(row: dict[str, object], index: int) -> BalancedSample:
    sample_id = parse_str(row.get("_id", f"sample_{index}"), "_id", index)
    dataset = parse_str(row.get("dataset"), "dataset", index)
    task = parse_str(row.get("task", ""), "task", index)
    context = parse_str(row.get("context", ""), "context", index)
    question = parse_str(row.get("question"), "question", index)
    answer_prefix = parse_str(row.get("answer_prefix", ""), "answer_prefix", index)
    answers = parse_answers(row.get("answers"), index)
    return BalancedSample(sample_id, dataset, task, context, question, answers, answer_prefix)


def parse_str(value: object, field: str, index: int) -> str:
    if not isinstance(value, str):
        raise RuntimeError(f"row {index} has non-string {field}")
    return value


def parse_answers(value: object, index: int) -> list[str]:
    match value:
        ...
        case str() as answer:
            return [answer]
        case _:
            raise RuntimeError(f"row {index} has invalid answers")
...
def extract_one(
    ...
    return {
        ...
        "teacher_formula": "sum_commit mean_heads softmax(Q_suffix K_[prompt,suffix]^T / sqrt(d)) sliced_to_prompt",
