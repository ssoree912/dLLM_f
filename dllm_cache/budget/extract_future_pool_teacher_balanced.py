...
DEFAULT_OUTPUT_ROOT: Final = Path("results/budget/future_pool_teacher_balanced_g128_top128")
...
def load_filtered_samples(config: ExtractFuturePoolConfig) -> list[BalancedSample]:
    ...
    selected = select_samples(candidates, config)
    if not selected:
...
def select_samples(candidates: Sequence[BalancedSample], config: ExtractFuturePoolConfig) -> list[BalancedSample]:
    if config.samples_per_dataset > 0:
        selected: list[BalancedSample] = []
        for dataset in config.datasets:
            dataset_samples = [sample for sample in candidates if sample.dataset == dataset]
            selected.extend(dataset_samples[config.skip_matching_samples : config.skip_matching_samples + config.samples_per_dataset])
        return selected
    selected = list(candidates[config.skip_matching_samples :])
