...
TWOHEAD_OUT="${TWOHEAD_OUT:-${OUT_ROOT}/attention_delta_2head_rank0p1_topk0_e${EPOCHS}}"
...
echo "[plan] two_head=${TWOHEAD_OUT}"
...
if [[ ! -f "${TWOHEAD_OUT}/checkpoint-best/pytorch_model.bin" ]]; then
  echo "[train] attention+delta 2-head scorer"
  "${PYTHON}" -m dllm_cache.budget.train_student \
    --teacher-root "${TEACHER}" \
    --output-dir "${TWOHEAD_OUT}" \
    --model "${MODEL}" \
    --datasets samsum \
    --val-ratio 0.1 \
    --epochs "${EPOCHS}" \
    ...
    --proj-dim 256 \
    --mlp-dim 512 \
    >"${LOG_DIR}/train_attention_delta_2head.log" 2>&1
else
  echo "[train] two-head checkpoint exists; skip"
fi
...
echo "[done] two_head=${TWOHEAD_OUT}/checkpoint-best"
