...
from revealed_answer.extract_teacher import ExtractConfig, extract_one, load_model_and_tokenizer
...
def main() -> int:
    ...
    with sample_path.open("w", encoding="utf-8") as handle:
        for index, sample in enumerate(samples, start=1):
            ...
            handle.flush()
            print(
                f"[oracle {index}/{len(samples)}] f1={result['f1']:.5f} "
                f"prompt={result['prompt_length']} pred={result['prediction']!r}",
                flush=True,
            )
    summary = {
        "task": "2wikimqa",
        "teacher": "gold_revealed_answer",
...
def build_runtime(config: EvalConfig) -> EvalRuntime:
    ...
    model, tokenizer = load_model_and_tokenizer(teacher_config)
    return EvalRuntime(model=model, tokenizer=tokenizer, teacher_config=teacher_config)
...
def run_one_sample(
    ...
    teacher_record = extract_one(runtime.model, sample, example, runtime.teacher_config)
    teacher_scores = teacher_record["teacher_norm"].float()
