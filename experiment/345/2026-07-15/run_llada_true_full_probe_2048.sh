...
LIMIT="${LIMIT:-50}"
TASKS="${TASKS:-2wikimqa hotpotqa triviaqa lcc samsum}"
...
run_task() {
  ...
  local output_path="${EXPERIMENT_ROOT}/results/llada_true_full_${task}_mlen${MODEL_MAX_LENGTH}_${limit_label}"
  local cmd=(
  ...
  printf '[start] true_full task=%s gen_length=%s limit=%s output=%s\n' "${task}" "${gen_length}" "${limit_label}" "${output_path}"
  cd "${REPO_ROOT}"
  "${cmd[@]}"
}
