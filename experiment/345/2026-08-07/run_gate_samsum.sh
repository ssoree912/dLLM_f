...
echo "=== START gate samsum $(date -Is)"
.venv/bin/python experiment/345/2026-08-07/refresh_gate.py \
  --model "$M" --student "$S" --output "$OUT/samsum.json" \
  --task samsum --samples 30 \
  --methods full dynkv oracle --refresh-intervals 1 4 --refresh-tokens 240 0
echo "=== END gate samsum $(date -Is)"
