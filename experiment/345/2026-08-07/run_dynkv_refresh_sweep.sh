#!/usr/bin/env bash
# dynkv 960 (프롬프트 50%) refresh sweep: refresh_interval=8, 16 (순차)
# 보고서 #7 계열, refresh 만 변경. 그 외 설정 동일.
...
run () {
  local refresh="$1"
  local name="dynkv_960_refresh${refresh}"
  local output="/home/M2026107/.cache/lmeval_samsum_b960_20260807/${name}"
  ...
  .venv/bin/python -m accelerate.commands.accelerate_cli launch \
    ...
    --model_args "pretrained=${MODEL_PATH},is_feature_cache=False,is_cfg_cache=False,max_length=2048,student_path=${STUDENT},student_prompt_dynamic_kv=True,student_budget=960,student_selection_mode=global,student_refresh_interval=${refresh},student_question_window=128" \
