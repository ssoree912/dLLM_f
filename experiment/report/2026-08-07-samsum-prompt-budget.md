...
layer-split은 10.1초/샘플로 근접하면서 **프롬프트 KV 메모리가 절반**이다.
...
| refresh_interval | ROUGE-L | origin 대비 |
|---:|---:|---:|
| 1 (매 step) | 0.3682 ± 0.0138 | 0.960 |
| **2** | **0.3715 ± 0.0140** | **0.969** |
| ∞ (완전 동결) | 0.2071 ± 0.0133 | 0.540 |

갱신을 절반으로 줄여도 품질이 떨어지지 않는다(오차 범위 내에서 오히려 높다). 그런데 완전히
얼리면 무너진다. 곡선이 매우 비선형이라 어딘가에서 급격히 꺾인다.
