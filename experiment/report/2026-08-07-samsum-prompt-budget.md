...
| refresh_interval | 생성 중 갱신 횟수 | ROUGE-L | origin 대비 |
|---:|---:|---:|---:|
| 1 (매 step) | 128 | 0.3682 ± 0.0138 | 0.960 |
| **2** | 64 | **0.3715 ± 0.0140** | **0.969** |
| **4** | 32 | **0.3822 ± 0.0142** | **0.997** |
| **8** | 16 | **0.3757 ± 0.0139** | **0.980** |
| **16** | 8 | **0.3758 ± 0.0137** | **0.980** |
| ∞ (완전 동결) | 0 | 0.2071 ± 0.0133 | 0.540 |

갱신 주기를 1→16으로 늘려도 품질은 전혀 떨어지지 않는다. refresh=4~16 구간(생성 128 step 동안
갱신 8~32회)은 모두 origin의 98~100%이며 오차 범위 내에서 오히려 refresh=1보다 높다. 그런데
완전히 얼리면(∞) 0.540으로 무너진다. **곡선은 refresh=16과 ∞ 사이에서만 급격히 꺾인다** —
즉 stale 손실을 피하는 데 필요한 갱신은 매우 드물어도 되고, 128 step에 8회 갱신으로 충분하다.
이는 3.5절의 layer 축 결과(얕은 layer의 드리프트가 작아 얼려도 무손실)와 같은 방향으로,
**프롬프트 K/V의 재계산 대부분이 잉여**임을 시간축에서도 확인해 준다.
